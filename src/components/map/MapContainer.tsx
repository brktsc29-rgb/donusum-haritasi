...
export function MapContainer({
    ...
    provider.initialize(containerRef.current, DEFAULT_MAP_OPTIONS).then(() => {
      ...
      if (searchInputRef.current) {
        ;(provider as GoogleMapsProvider).initSearchBox(
          ...
          (latlng) => { provider.setCenter(latlng); provider.setZoom(17) }
        )
      }
    })
    ...
    if (showParcels) {
      parcels.forEach((parcel) => {
        if (parcel.coordinates.length < 3) {
          if (parcel.center) provider.addMarker(parcel.id, parcel.center, parcel.parcel_no, colorStatusToHex(parcel.color_status))
          return
        }
      ...
      parcels.forEach((p) => {
        if (!blockMap.has(p.block_no)) blockMap.set(p.block_no, [])
        blockMap.get(p.block_no)!.push(p)
      })
      blockMap.forEach((blockParcels, blockNo) => {
        ...
        if (withCenter?.center) provider.addMarker(`block-${blockNo}`, withCenter.center, blockNo, color)
      })
    }

    if (adaCoords && adaCoords.length >= 3) {
      provider.addPolygon('temp-ada', adaCoords, POLYGON_STYLES.ada)
    }
  }, [parcels, showParcels, onParcelClick, adaCoords])

  // Fit to ada bounds once when it first appears
  useEffect(() => {
    if (adaCoords && adaCoords.length >= 3) {
      if (!adaFittedRef.current) {
        providerRef.current?.fitCoords(adaCoords)
  ...
  const handleFinish = useCallback(() => providerRef.current?.finishDrawNow(), [])
  const handleUndo = useCallback(() => providerRef.current?.undoLastDrawPoint(), [])
  ...
  return (
    <div className="relative w-full h-full">
        ...
        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 w-72 sm:w-96 max-w-[calc(100%-5rem)]">
          <div className="relative">
            ...
            <input
              ref={searchInputRef}
              type="text"
              placeholder="Adres ara, Enter'a bas"
      ...
      {drawMode && (
        <>
          <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-primary text-primary-foreground text-xs px-4 py-2 rounded-full shadow-lg z-10 pointer-events-none text-center max-w-[calc(100%-2rem)]">
            {drawLabel ?? 'Köşe noktalarına dokun'} • {drawPointCount} nokta
          </div>
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-10">
            <Button size="lg" variant="outline" className="bg-white shadow-lg h-12 px-4 gap-1.5" onClick={handleUndo} disabled={drawPointCount === 0}>
              <Undo2 className="h-4 w-4" />
              <span className="text-sm">Geri Al</span>
            </Button>
            <Button size="lg" className="bg-green-600 hover:bg-green-700 text-white shadow-lg h-12 px-5 gap-1.5" onClick={handleFinish} disabled={drawPointCount < 3}>
              <Check className="h-4 w-4" />
              <span className="text-sm">Bitir</span>
            </Button>
            <Button size="lg" variant="destructive" className="shadow-lg h-12 px-4 gap-1.5" onClick={handleCancel}>
              <X className="h-4 w-4" />
